...
| 2026-07-13 | `0077e5c` | #204, #205, #206, #207, #208 | reduction round-trips · `interpret_chord` mask consistency · determinism · display-free analysis · transpose invariance · newest surfaces (search/repair, patterns/, temporal part-content profiles, corpus exercise regime) · semantic coherence (§6a) | First scheduled cycle. `pytest audit/checks` (3) and `pytest tests/` (958) both green pre-audit. Skipped PR #203 (`gap-e-slice2-relation-atoms`, `part_relations.py`) as in-flight per §5. Findings: #204 med (io/midi sub-tick-duration write bug, invalid byte order + misclassified loss), #205 med (`scale_analysis.step_pattern` breaks transpose invariance, ignores `tonic_pc`), #206 low (`part_profiles` O(n²) onset clustering), #207 low (`part_profiles` unbounded `onset_density` on near-zero-duration input), #208 low/docs (ROADMAP.md self-contradicts on the repair soft-score contract, "improves" vs. shipped "must not worsen"). All five independently re-verified by executing each repro before filing. |
