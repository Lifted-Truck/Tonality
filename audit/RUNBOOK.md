...
| 2026-08-17 | `17fbc95` | #274 (high, efficiency), #275 (med, doctrine/reporting), #276 (med, §6a coherence), #277 (low, docs) | newest-surface correctness (gap 33, 31b, 26, Phase 7 slice 0) · §6b efficiency (structural read + `scaling_probe`) · §6a semantic coherence · standard invariant sweep on `core/`, `theory/functions.py`, `search/`, `patterns/`, `rules/`, `representation/` | `pytest tests/` (1210) + `audit/checks/` (3) green pre-cycle. #274: `segment_to_chords`/`_window_pc_weights` rescans all events per window (O(windows×events), fitted exponent ≈1.5-1.65 on synthetic 500-8000 note sequences) — now hot on 3 new gap-25/26 callers, plus `classify_chromatic_events` calls it twice redundantly. #275: `remap_by_degree`/`modal_transform` invent an unflagged octave at the MIDI 0/127 boundary, contradicting their own "octaves never invented" docstring claim — same root cause as #262 but in files PR #273 didn't touch. #276: `retonicize`'s MCP docstring says "ANALYSIS-side" while its package (`mts/generate/__init__.py`) claims everything in it "CHOOSEs pitches" and its own module docstring (`remap.py`) says it makes zero edits — three-way contradiction. #277: `mts/generate/` (Phase 7 slice 0) is absent from CLAUDE.md's architecture-layer diagram entirely. Standard invariant sweep (catalog integrity, reduction round-trips, interpret_chord mask consistency, display-free analysis, transpose invariance, determinism, functional-harmony error-not-guess) came back clean on the older/broader surface — no findings there. Skipped `mts/generate/conform.py`'s MIDI-boundary bound claim specifically (already covered by open PR #273 / issue #262). Added `segment_to_chords` to `scaling_probe.py`'s `__main__` as this cycle's newest scalable entry point (charter §6b already named it in the must-stay-linear set; it just wasn't wired into the probe harness yet). |
