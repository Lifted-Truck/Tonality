...
| 2026-07-27 | 649ac46 | #238, #239 | committed checks (regression floor) · newest-surface correctness (gap-27 path-traversal fix, gap-23/#230/#231 budget-polarity gating, gap-E-slice-4 cross-part patterns) · §6a semantic coherence · §6b efficiency/scaling | No open PRs at scope time. #238 (low): `search/repair._implicated_notes` rescans `sequence.events` per violation per DFS node (O(V·E)/node) — same anti-pattern class as #206/#214; bounded by `max_evaluations` but not linear per node. #239 (low, docs): `mcp/tools.validate_ruleset`/`evaluate_ruleset` docstrings never updated for the gap-23 `budget` polarity / `budgets_hold` field. Verified clean (no issue filed): path-traversal fix (be26ad8) is complete — the one other untrusted-path site (`midi_file_analysis`/`piano_roll_view`) is already tracked as an open ROADMAP decision, not an omission; budget-polarity *gating* logic (repair, combine/specialize/diff) is correct post-#230/#231; cross-part pattern matcher (03fcc13) — degree-without-key errors, chordal voices skipped-and-named, register re-derived per onset (not fixed voice→line), overlapping occurrences all reported, ~O(E log E) in event count (added as a new entry in `scaling_probe.py`, exponent≈1.10); "quota" vocabulary surviving in `integrations/style-manifold/seed.md` + `tests/` is intentional (ROADMAP explicitly keeps "née quota" for commit-history continuity; #231's "zero quota hits" claim was scoped to `mts/` only and holds). |
