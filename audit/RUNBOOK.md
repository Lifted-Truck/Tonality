...
| 2026-08-03 | `5c55f58` | #245 (high, mcp packaging), #246 (med, efficiency), #247 (med, semantic/RE-3d recurrence), #248 (low, docs), #249 (low, docs) | catalog integrity, reduction round-trips, determinism, display-free analysis (drum patterns) · efficiency §6b structural read + scaling probe (drums/parts/relations/rules/repair/mcp tools) · semantic coherence §6a (README tool count, ROADMAP repair contract, tool-manifest-pin, vocabulary) · MCP security fixes re-attacked (tool-manifest pin, path traversal) | First scheduled cycle. No open PRs, no prior audit issues. #245: `mcp>=1.2` unbounded breaks `build_server()` against current PyPI `mcp==2.0.0` (FastMCP renamed), invisible to CI (`.[dev]`-only install). #246: `drums.py _match()` rescans onsets per bar per pattern, exponent≈1.81 (same class as #206/#214). #247: `repair_sequence.already_conformant` reintroduces the RE-3d "held vs never-tested" conflation one layer above the evaluator. #248/#249: doc drift (tool count 57→69; repair contract missing budget-gating post-#230). Both MCP security hardenings (tool-manifest pin, path-traversal fix) re-attacked and held. Dev suite (`pytest tests/`, 1092) and `audit/checks` (3) green at HEAD once local venv's `mcp` pinned to 1.29.x (2.0.0 breaks the build_server test per #245).
