...
| **Set-class identity** | normal order, Rahn prime form, Z-partners, **DFT magnitudes** (a 6-D "harmonic color" embedding: \|f₅\|≈diatonicity, \|f₆\|≈whole-tone-ness) — plus, via `set_class_info` (A6 brief-15): **DFT phases** `arg(f₁..f₆)` (Tn-variant + inversion-negating — the hue/handedness the magnitudes discard) and two chirality scalars: **`trichord_chirality`** (step-gap `(a−b)(b−c)(c−a)`: major −2 / minor +2 / achiral 0 / null past 3 notes) and **`general_chirality`** = `Im(f₁·f₂·conj(f₃))` (bispectrum slice; any cardinality; Tn-invariant, inversion-odd; major<0/minor>0; 0 for achiral; **separates dom7 ↔ m7♭5**). \|f₅\| consonance = `dft_magnitudes[4]`; prime-form + 12-bit `mask` returned |
