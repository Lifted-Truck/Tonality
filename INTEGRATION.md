...
| **Set-class identity** | normal order, Rahn prime form, Z-partners, **DFT magnitudes** (a 6-D "harmonic color" embedding: \|f₅\|≈diatonicity, \|f₆\|≈whole-tone-ness) — plus, via `set_class_info` (A6 brief-15): **DFT phases** `arg(f₁..f₆)` (Tn-variant + inversion-negating — the hue/handedness the magnitudes discard) and three handedness scalars: **`trichord_chirality`** (step-gap `(a−b)(b−c)(c−a)`: major −2 / minor +2 / achiral 0 / null past 3 notes), **`general_chirality`** = `Im(f₁·f₂·conj(f₃))` (bispectrum slice; any cardinality; Tn-invariant, inversion-odd; major<0/minor>0; 0 for achiral; **separates dom7 ↔ m7♭5**; smooth magnitude but false-zeros on a few exotic classes), and **`chirality_sign`** ∈ {−1,0,+1} — the **complete** handedness, **0 iff achiral**, inversion-odd, agreeing with general_chirality where nonzero (bispectrum slices + one `Im(f₁³·conj(f₃))` trispectrum fallback; verified exhaustively over all 4096 masks). \|f₅\| consonance = `dft_magnitudes[4]`; prime-form + 12-bit `mask` returned |
