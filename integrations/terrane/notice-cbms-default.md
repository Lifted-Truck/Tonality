...
2. **Pin the old contract (zero change to your behaviour):** pass
   **`profile_version="kk-1982.1"`** to `infer_key` / `key_induction` /
   `key_tracking` / `structural_keys` / `midi_file_analysis`. This reproduces the
   exact pre-flip margins — the selector makes the old behaviour a one-arg opt-out,
   so **nothing breaks** if you pin.
