...
class Scale:
    ...
    @classmethod
    def from_degrees(cls, name: str, degrees: Iterable[int]) -> "Scale":
        normalized = tuple(sorted({validate_pc(int(pc) % 12) for pc in degrees}))
        mask = mask_from_pcs(normalized)
        return cls(name=name, degrees=normalized, mask=mask)
    ...
    def transpose(self, semitones: int) -> "Scale":
        ...
        return Scale.from_degrees(f"{self.name}+{semitones}", transposed_degrees)
